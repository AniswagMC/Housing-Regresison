...
kable(mses_all_models, label = "MSE Of All Models on Validation Set")
````
...
lasso.test.pred <- predict(reg_lasso, test)
ridge.test.pred <- predict(reg_ridge, test)
test_all_models <- tibble(
  Actual = test$spotify_track_popularity,
  `Random Forest` = rf.test.pred, 
  `Linear Regression` = lm.test.pred,
  `LASSO` = lasso.test.pred,
  `RIDGE`= ridge.test.pred
)
...
### Results Discussion
Comparing the results from all the models explored, both individual and combined, we can clearly see that we get the best predictions from our combined model with an accuracy of [INSERT VALUE]. This was expected as ensemble methods tend to perform better and are more robust in nature than individual algorithms. The second best model is the [INSERT REGRESSION] model, with accuracy [INSERT VALUE].

### Limitations and Further Work
We can see that the general accuracy of results increase as the complexity of our models increased. This tells us that the true model may follow a non-linear trend. Furthermore, with transformation or scaling the values of certain variables, the predictions could have been further improved. So, it is worth exploring using other non-linear models such as polynomial regressions or neural networks. Due to a lack of time, we were unable to look into other such methods for our regression analysis.
